...
class Base64ImageField(serializers.ImageField):
    ...
    def to_internal_value(self, data):
        ...
        if isinstance(data, str) and data.startswith('data:image'):
            ...
            data = ContentFile(base64.b64decode(image_string), name='img.' + ext)
...
class RecipeCreateSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        self.create_update_ingredients_amount(ingredient_recipe=ingredient_recipe, recipe=recipe)
        return recipe
    ...
    def update(self, instance, validated_data):
        ...
        if not ingredient_recipe:
            raise serializers.ValidationError('Добавьте минимум один ингредиент')
        instance.ingredient_recipe.all().delete()
        self.create_update_ingredients_amount(ingredient_recipe=ingredient_recipe, recipe=instance)
        instance.save()
        return instance

    def to_representation(self, instance):
        return RecipeGetSerializer(instance, context={'request': self.context.get('request')}).data
